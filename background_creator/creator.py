...
class Creator:
    def __init__(self, aspect_ratio: str = "4/5", orientation: str = "vertical", img: Optional[str] = None, output_dir: str = './', verbose: bool = False) -> NoReturn:
            ...
            if result:
                ...
                if len(aspect_ratio_dimensions) == 2:
                    ...
                    if self.orientation == "vertical":
                        ratio = aspect_ratio_dimensions[0] / \
                            aspect_ratio_dimensions[1]
                    elif self.orientation == "horizontal":
                        ratio = aspect_ratio_dimensions[1] / \
                            aspect_ratio_dimensions[0]
    ...
    def _clrd_bg(self, img: Image = None, color: Optional[tuple[int]] = (0, 0, 0)) -> Image:
        img = img or self.img
        src_w, src_h = img.size
        bg_w = int(src_h * self.ratio)
    ...
    def _brtst_c_bg(self, img: Image = None) -> Image:
        img = img or self.img
    ...
    def generate(self, img=None, background_type: Optional[str] = "BLURRED", save: Optional[bool] = False, save_folder: Optional[str] = None, show: Optional[bool] = False, *args, **kwargs):
        ...
        try:
            ...
            if type(img) == str:
                ...
                raise ValueError(
                    '"img" should be either a str or PIL.Image')
            if save:
